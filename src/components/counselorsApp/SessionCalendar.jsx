...
const MeetingCalendar = () => {
    ...
    const sessionDates = [
        { date: '2024-07-05', details: 'Session with John Doe', time: '10:00 AM' },
        { date: '2024-07-10', details: 'Group therapy session', time: '2:00 PM' },
        { date: '2024-07-15', details: 'Session with Jane Smith', time: '11:30 AM' },
        { date: '2024-07-18', details: 'Supervision meeting', time: '3:00 PM' },
        { date: '2024-07-25', details: 'Workshop: Stress Management', time: 'All day' },
    ];


    const tileClassName = ({ date }) => {
        return sessionDates.some(meeting => isSameDay(new Date(meeting.date), date)) 
            ? 'bg-indigo-100 text-indigo-800 hover:bg-indigo-200 cursor-pointer rounded-full' 
            : 'hover:bg-gray-100 rounded-full';
    };

    const getMeetingDetails = (date) => {
        return sessionDates.filter(meeting => isSameDay(new Date(meeting.date), date));
    };
