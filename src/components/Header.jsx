...
const Header = () => {
  ...
  return (
    <header
      className={`fixed h-20 w-full top-0 left-0 z-[10] transition-colors duration-500 ${
        scrolled ? "bg-gray-700 shadow-lg" : "bg-transparent"
      }`}>
      <div className="container mx-auto flex justify-between items-center py-4 px-8 z-50">
        ...
        <button
          ...
          className="bg-gray-700 px-4 py-2 hover:bg-gray-700 text-white rounded-md cursor-pointer transition duration-300 ease-in-out hidden lg:block">
          Login
        </button>
        ...
        <button
          onClick={toggleMenu}
          className="lg:hidden text-white focus:outline-none z-[21]">
          {menuOpen ? (
      ...
      <nav
        className={`pt-20 lg:hidden fixed top-0 left-0 w-full bg-gray-700 text-white transition-transform duration-300 ${
          menuOpen ? "transform translate-y-0" : "transform -translate-y-full"
        }`}>
        <button
          onClick={handleLoginClick}
          className="block py-4 px-6 text-left border-b border-blue-400 hover:bg-gray-700 w-full transition duration-200 ease-in cursor-pointer">
          Login
        </button>
        {links.map((link) => (
          <Link
